...
def preprocess_graph(adj, layer, norm='sym', renorm=True):
    ...
    for i in range(len(reg)):
        adjs.append(2*ident - (reg[i] * laplacian))
...
def clustering(feature, true_labels, cluster_num):
    ...
    # acc, nmi, ari, f1 = eva(true_labels, predict_labels, show_details=True)
...
def contrastive_loss(z1, z2, temperature=0.5):
    ...
    loss = -torch.log(sim_pos / (torch.sum(sim_neg, dim=-1) - sim_pos))
    return loss.mean()
